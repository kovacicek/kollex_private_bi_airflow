def run_full_load():
    ...
    for df_chunk in df_product:
        ...
        chunk.drop(
            ['title_2', 'net_content_2', 'brand_2', 'net_content_liter_2',
             'contact_info_2'], axis=1, inplace=True, errors='ignore')
        ...
        sku_category_fact = pd.read_sql_table(
            'sku_category_fact', con=pg_engine,
            schema=Variable.get("PG_INFO_SCHEMA"))
        chunk = chunk.merge(sku_category_fact, how='inner',
                            left_on='identifier', right_on='sku',
                            suffixes=('', '_y'))
        chunk.drop(chunk.filter(regex='_y$').columns.tolist(), axis=1,
        ...
        chunk = chunk.merge(
            attribute_options, left_on='net_content_liter', right_on='code',
            how='left')
        chunk['net_content_liter'] = chunk['value'].combine_first(
            chunk['net_content_liter']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='net_content_uom', right_on='code',
            how='left')
        chunk['net_content_uom'] = chunk['value'].combine_first(
            chunk['net_content_uom']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='type_single_unit', right_on='code',
            how='left')
        ...
        chunk = chunk.merge(
            attribute_options, left_on='amount_single_unit', right_on='code',
            how='left', )
        chunk['amount_single_unit'] = chunk['value'].combine_first(
            chunk['amount_single_unit']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='structure_packaging_unit',
            right_on='code', how='left')
        chunk['structure_packaging_unit'] = chunk['value'].combine_first(
            chunk['structure_packaging_unit']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='type_packaging_unit', right_on='code',
            how='left')
        chunk['type_packaging_unit'] = chunk['value'].combine_first(
            chunk['type_packaging_unit']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='golden_record_level1', right_on='code',
            how='left')
        chunk['golden_record_level1'] = chunk['value'].combine_first(
            chunk['golden_record_level1']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='status_base', right_on='code',
            how='left')
        chunk['status_base'] = chunk['value'].combine_first(
            chunk['status_base']).astype(str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')

        chunk = chunk.merge(
            attribute_options, left_on='brand', right_on='code', how='left')
        chunk['brand'] = chunk['value'].combine_first(chunk['brand']).astype(
            str)
        chunk.drop(attribute_options.columns, inplace=True, axis=1,
                   errors='ignore')
        ...
        for merchant in sorted_merchants['merchant_key']:
            chunk[str(merchant) + '_enabled'] = chunk[
                'raw_values_product'].apply(lambda x: json.loads(x)[
                'gfgh_' + str(merchant) + '_enabled']['<all_channels>'][
                '<all_locales>'] if 'gfgh_' + str(
                merchant) + '_id' in json.dumps(x) else False).astype(str)
            chunk[str(merchant) + '_id'] = chunk['raw_values_product'].apply(
                lambda x: json.loads(x)['gfgh_' + str(merchant) + '_id'][
                    '<all_channels>']['<all_locales>'] if 'gfgh_' + str(
                    merchant) + '_id' in json.dumps(x) else None).astype(str)
            chunk[str(merchant) + '_freigabe'] = chunk[
                'raw_values_product'].apply(lambda x: json.loads(x)[
                'freigabe_' + str(merchant) + '_id']['<all_channels>'][
                '<all_locales>'] if 'freigabe_' + str(
                merchant) + '_id' in json.dumps(x) else None).astype(str)
            # print(merchant)
        ...
        enabled_df['enablement'] = enabled_df[(enabled_df == 'True') | (
                    enabled_df == 'true') | (enabled_df == 'TRUE')].count(
            axis=1) - 2
        ...
        chunk.drop(['raw_values_model', 'raw_values', 'raw_values_product'],
                   axis=1, inplace=True, errors='ignore')
        chunk.drop(['family_id', 'product_model_id', 'family_variant_id'],
                   axis=1, inplace=True, errors='ignore')
        ...
        chunk.to_sql(pg_tables_to_use, pg_engine, schema=pg_schema,
