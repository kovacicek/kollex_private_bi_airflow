...
from include.db import prepare_pg_connection, prepare_mysql_akeneo_connection
...
def run_delta_load():
    ...
    connection.execute(
        f"delete from  {pg_raw_schema}.{pg_tables_to_use} where identifier  in {identifiers} ;"
    )
    ...
    sku_category_fact = pd.read_sql_table(
        "sku_category_fact", con=pg_engine, schema=pg_info_schema
    )
    chunk = chunk.merge(
        sku_category_fact,
        how="inner",
    ...
    for merchant in sorted_merchants["merchant_key"]:
        chunk[str(merchant) + "_enabled"] = chunk["raw_values_product"].apply(
            lambda x: json.loads(x)["gfgh_" + str(merchant) + "_enabled"][
                "<all_channels>"
            ]["<all_locales>"]
        ...
        chunk[str(merchant) + "_freigabe"] = chunk["raw_values_product"].apply(
            lambda x: json.loads(x)["freigabe_" + str(merchant) + "_id"][
                "<all_channels>"
            ]["<all_locales>"]
